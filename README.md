...
> 🔬 Статус: **Phase 1 — бейзлайны считаются на полной вселенной активов; статистический слой готов**
...
**Полный грид бейзлайнов (выполняется).** 21 серия × 7 моделей × 3 горизонта × 3 уровня α на всех 12 ядрах; результат — `results/baselines_full.parquet`.
...
- [x] **Phase 1 — Данные и классические бейзлайны**: кэширующие загрузчики, безопасная к утечкам агрегация доходностей; historical, FHS, EWMA, GARCH-N/-t, GJR-t, HAR; walk-forward движок с доказательством отсутствия утечек; параллельный грид-раннер *(полный прогон выполняется; остались data card и RV-подмножество)*
...
- [x] **Phase 3 — Оценка и статистика** *(построена досрочно)*: QLIKE, FZ0, Kupiec, Christoffersen, DQ, Acerbi–Székely Z2, Diebold–Mariano + HLN, Model Confidence Set; всё валидировано по размеру/мощности на синтетике *(осталось: база результатов + генераторы таблиц и графиков)*
